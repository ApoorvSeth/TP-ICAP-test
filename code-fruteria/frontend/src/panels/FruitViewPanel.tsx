...
import { Card, Form, Select, InputNumber, Button, Typography, List, message } from 'antd';
...
export const FruitViewPanel: React.FC = () => {
  ...
  const handleBuy = () => {
    if (machine.buy(selectedFruit, amount)) {
      setMessageText(`Bought ${amount} ${selectedFruit}(s).`);
      messageApi.success(`Bought ${amount} ${selectedFruit}(s).`);
    } else {
      setMessageText(`Not enough ${selectedFruit}s in inventory.`);
      messageApi.error(`Not enough ${selectedFruit}s in inventory.`);
    }
  ...
  const handleSell = () => {
    ...
    setMessageText(`Sold ${amount} ${selectedFruit}(s).`);
    messageApi.info(`Sold ${amount} ${selectedFruit}(s).`);
    setInventory(machine.getInventory());
  };
        ...
        <div style={{ minHeight: 24, marginBottom: 16 }}>
          {message && (
            <Text
              strong
              style={{
                color: messageText.startsWith('Bought')
                  ? '#52c41a'
                  : messageText.startsWith('Not enough')
                  ? '#f5222d'
                  : undefined
              }}
            >
        ...
        <List
          ...
          renderItem={fruit => (
            <List.Item style={{ padding: '4px 0' }}>
              <Text style={{ color: styles.textColor, fontSize: 16 }}>
                {fruit}: <Text strong style={{ color: styles.textColor }}>{inventory[fruit]}</Text>
              </Text>
            </List.Item>
          )}
        />
